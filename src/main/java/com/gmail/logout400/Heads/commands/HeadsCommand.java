...
public class HeadsCommand implements CommandExecutor {
    ...
    public boolean headsCommand(CommandSender sender, org.bukkit.command.Command command, String label, String[] args) {
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("version")) {
            SimpleSkull.sendMessage(sender, msgs.pluginVersion.replaceAll("%version%", "&e" + plugin.getVersion() + "&a"));
            return true;
        }
    ...
    private final static HashSet<Material> materials = new HashSet<Material>(1) {
        {
            add(Material.AIR);
        }
    };
    ...
    public void helpMessage(CommandSender sender) {
        SimpleSkull.customMessage(sender, "&e--------------------[ &b&l Heads v" + plugin.getVersion() + "&r&e ]-------------------");
        if (sender instanceof Player) {
        ...
        if (sender instanceof Player) {
            SimpleSkull.sendMessage(sender, "&7/heads &e- &aopens a menu of heads.");
